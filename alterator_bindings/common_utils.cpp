...
std::string UnUtf8(const std::string &str) noexcept {
    ...
    try {
        ...
        while (ind < str.size()) {
            ...
            // 2 bytes - skip
            if ((str.at(ind) & 0b11100000) == 0b11000000 &&
                ind < str.size() - 1) {
                ind += 2;
                continue;
            }
            // 3 bytes - skip
            if ((str.at(ind) & 0b11110000) == 0b11100000 &&
                ind < str.size() - 2) {
                ind += 3;
                continue;
            }
...
std::string QuoteIfNotQuoted(const std::string &str) noexcept {
    if (str.empty()) return "\"\"";
    if (str[0] != '\"' || str.back() != '\"') return WrapWithQuotes(str);
    return str;
}
...
std::optional<uint32_t> StrToUint(const std::string &str) noexcept {
    uint32_t res = 0;
    try {
        if (!str.empty() && str[0] == '-') throw std::exception();
        size_t pos = 0;
        res = static_cast<uint32_t>(std::stoul(str, &pos, 10));
        if (pos != str.size()) throw std::exception();
