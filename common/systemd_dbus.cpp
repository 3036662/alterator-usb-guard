...
std::optional<bool> Systemd::IsUnitEnabled(
    const std::string &unit_name) noexcept {
    if (!Health()) return std::nullopt;
    std::string result;
    try {
        const auto proxy = CreateProxyToSystemd(kObjectPath);
...
std::optional<bool> Systemd::IsUnitActive(
    const std::string &unit_name) noexcept {
    if (!Health()) return std::nullopt;
    std::string result;
    try {
        ...
        const auto proxy_unit = CreateProxyToSystemd(unit_path);
        const auto active_state = proxy_unit->getProperty("ActiveState")
...
std::optional<bool> Systemd::StartUnit(const std::string &unit_name) noexcept {
    ...
    try {
        ...
        for (int i = 0; i < 10; ++i) {
            ...
            if (isActive.has_value() && isActive.value()) return true;
        }
...
std::optional<bool> Systemd::EnableUnit(const std::string &unit_name) noexcept {
    ...
    try {
        ...
        for (int i = 0; i < 10; ++i) {
            ...
            if (isEnabled.has_value() && isEnabled.value()) return true;
        }
...
std::optional<bool> Systemd::DisableUnit(
    const std::string &unit_name) noexcept {
    if (!Health()) return std::nullopt;
    try {
        std::this_thread::sleep_for(std::chrono::milliseconds(200));
...
std::optional<bool> Systemd::RestartUnit(
    const std::string &unit_name) noexcept {
    if (!Health()) return std::nullopt;
...
bool Systemd::Health() noexcept {
    if (!connection_) ConnectToSystemDbus();
    return static_cast<bool>(connection_);
}

std::unique_ptr<sdbus::IProxy> Systemd::CreateProxyToSystemd(
    const std::string &path) const {
    return sdbus::createProxy(*connection_,
                              sdbus::ServiceName{kDestinationName},
