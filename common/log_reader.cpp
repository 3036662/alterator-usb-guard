...
LogReader::LogReader(const std::string &fpath) : log_file_path_(fpath) {
    if (log_file_path_.empty()) throw std::invalid_argument("Empty filepath");
}

vecstring LogReader::GetFromFile(const vecstring &filters) const {
    vecstring res;
    if (log_file_path_.empty()) throw std::logic_error("An empty path to file");
...
PageData LogReader::GetByPage(const vecstring &filters, uint page_number,
    ...
    try {
        ...
        int index_last =
            lines_size - static_cast<int>(page_number * pages_size);
        int index_first =
            lines_size - static_cast<int>((page_number + 1) * pages_size);
        index_first = std::max(index_first, 0);
        index_last = std::min(index_last, lines_size);
        if (index_first >= index_last) return data;
