...
void Test::Run1() {
    ...
    if (os.is_open()) os << "blalala usb blabla authorized" << std::endl;
    os.close();

    // not usb neither authorized
    os = std::ofstream(file2);
    if (os.is_open()) os << "bla bla \n bla bla" << std::endl;
    os.close();

    // simple long file
    os = std::ofstream(file3);
    if (os.is_open()) {
        for (int i = 0; i < 2048; ++i) os << i;
    }
    os.close();

    // only usb
    os = std::ofstream(file4);
    if (os.is_open()) os << "bla usb \n bla bla" << std::endl;
    os.close();
...
void Test::Run2() {
    ...
    Log::Test() << " ACTIVE " << cs.guard_daemon_active_ << " ENABLED "
                << cs.guard_daemon_enabled_ << " DAEMON_OK "
                << cs.guard_daemon_OK << " UDEV_RULES_OK " << cs.udev_rules_OK_;
...
void Test::Run7() {
    ...
    std::vector<std::string> res = FoldUsbInterfacesList(
        "with-interface { 0e:01:00 0e:02:00 0e:02:00 0e:02:00 0e:02:00 "
        "0e:02:00 "
...
void Test::Run10() {
    std::string str = "allow name \"name\"";
    {
        ...
        assert((parser.target() == guard::Target::allow) && !parser.vid_ &&
               !parser.pid_ && !parser.hash_ && !parser.serial_ &&
               !parser.port_ && !parser.with_interface_ && !parser.cond_);
    }

    str =
        "allow hash \"0,;UW~fn\" id  7Dbb:* serial \"R0Ci+jH\'\" parent-hash  "
    ...
    str = "block id 8564:1000";
    {
        ...
        assert((parser.target() == guard::Target::block) &&
               *parser.vid_ == "8564" && *parser.pid_ == "1000" &&
               !parser.hash_ && !parser.device_name_ && !parser.serial_ &&
               !parser.port_ && !parser.with_interface_ && !parser.cond_);
    }
    str = "block id 8564:*";
    {
    ...
    str = "block id 8564:* hash 4Q3Ski/Lqi8RbTFr10zFlIpagY9AKVMszyzBQJVKE+c=";
    {
        ...
        assert((parser.target() == guard::Target::block) &&
               ...
               *parser.hash_ ==
                   "4Q3Ski/Lqi8RbTFr10zFlIpagY9AKVMszyzBQJVKE+c=" &&
               !parser.device_name_ && !parser.serial_ && !parser.port_ &&
               !parser.with_interface_ && !parser.cond_);
    }
    ...
    // name
    str =
        "block hash 4Q3Ski/Lqi8RbTFr10zFlIpagY9AKVMszyzBQJVKE+c= name \"USB "
        "name   _      Lastname\" ";
    {
        guard::GuardRule parser(str);
        assert((parser.target() == guard::Target::block) && !parser.vid_ &&
          ...
          WrapWithQuotes("2-2") + "} with-interface 08:06:50";
    {
        ...
        std::pair<guard::RuleOperator, std::vector<std::string>>
            expected_interface{guard::RuleOperator::no_operator,
                               interface_expected};
          ...
          WrapWithQuotes("2-2") + "} with-interface none-of{08:06:50 07:*:*}";
    {
        ...
        std::pair<guard::RuleOperator, std::vector<std::string>>
            expected_interface{guard::RuleOperator::none_of,
                               interface_expected};
