...
CsvRule::CsvRule(const rapidcsv::Document &doc, size_t index) {
    ...
    if (n_cols >= 2) interface_ = row[1];
    if (n_cols >= 3) vidpid_ = row[2];
    if (n_cols >= 4) hash_ = row[3];
...
std::string CsvRule::BuildString() const noexcept {
    ...
    if (!vidpid_.empty()) string_builder << "id " << vidpid_ << " ";
    if (!hash_.empty())
