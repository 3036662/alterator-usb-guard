...
class ConfigStatus : public SerializableForLisp<ConfigStatus> {
   public:
     ...
     * @return  std::pair<std::vector<GuardRule>,uint> Parsed rules,total lines
     * in file
     */
    std::pair<std::vector<GuardRule>, uint> ParseGuardRulesFile()
        const noexcept;
    ...
    /**
     * @brief Try to run usbguard, just for a health check. Leaves a daemon in
     * its initial state if parameter is false
   ...
   private:
     ...
     */
    std::optional<std::string> ExtractConfigFileName(
