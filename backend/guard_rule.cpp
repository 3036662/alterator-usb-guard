...
std::string GuardRule::ConditionsToString() const {
    ...
    if (map_operator.count(cond_->first) != 0) {
        ...
        res += " ";
        if (cond_->first != RuleOperator::no_operator) res += "{";
    }
    int counter = 0;
    for (const auto &rule_with_bool : cond_->second) {
        // look in conditions map
        if (map_conditions.count(rule_with_bool.second.first) == 0) continue;
        if (counter != 0) res += " ";
        // check for negotiation
        if (!rule_with_bool.first) res += '!';
...
std::string GuardRule::BuildString(
    bool build_parent_hash,
    bool with_interface_array_no_operator) const noexcept {
    std::ostringstream res;
    res << map_target.at(target_);
    if (vid_ && pid_) res << " id " << *vid_ << ":" << *pid_;
    if (serial_) res << " serial " << QuoteIfNotQuoted(*serial_);
    if (device_name_) res << " name " << QuoteIfNotQuoted(*device_name_);
    if (hash_) res << " hash " << QuoteIfNotQuoted(*hash_);
...
std::string GuardRule::InterfacesToString(
    bool with_interface_array_no_operator) const {
    std::stringstream res;
    if (!with_interface_.has_value()) return res.str();
...
vecPairs GuardRule::SerializeForLisp() const {
    ...
    if (level_ == StrictnessLevel::hash) {
        ...
        res.emplace_back(
            "lbl_rule_desc",
            device_name_.has_value() ? EscapeQuotes(device_name_.value()) : "");
    }
    // VID::PID
    if (level_ == StrictnessLevel::vid_pid) {
        ...
        res.emplace_back("lbl_rule_product", device_name_.has_value()
                                                 ...
                                                 : "");
        res.emplace_back("lbl_rule_port", port_.has_value()
                                              ? EscapeQuotes(PortsToString())
                                              : "");
    }
    // interface
    if (level_ == StrictnessLevel::interface) {
        res.emplace_back("lbl_rule_interface",
                         with_interface_.has_value()
...
StrictnessLevel GuardRule::StrToStrictnessLevel(
    const std::string &str) noexcept {
    if (str == "hash") return StrictnessLevel::hash;
    if (str == "vid_pid") return StrictnessLevel::vid_pid;
    if (str == "interface") return StrictnessLevel::interface;
...
RuleWithBool GuardRule::ParseOneCondition(
    ...
    if (it_range_beg != it_range_end && may_have_params) {
        ...
        if (!par_value.empty()) {
            rule_with_param.first =
                utils::ConvertToConditionWithParam(condition);
