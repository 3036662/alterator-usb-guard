...
std::string JsonChanges::Process(bool apply) {
    ...
    if (preset_mode_ == "put_connected_to_white_list_plus_HID") AddAllowHid();
    ...
    for (auto &rule : rules_to_add_) new_rules_.push_back(std::move(rule));
        ...
        for (const auto &rule : added_by_preset_) {
            switch (rule.level()) {
                case StrictnessLevel::hash: {
                    ...
                    tmp_hash["description"] =
                        common_utils::UnQuote(rule.device_name());
                ...
                case guard::StrictnessLevel::interface: {
                    ...
                    res["interface"].as_array().emplace_back(
                        std::move(tmp_interface));
                } break;
                case guard::StrictnessLevel::vid_pid: {
                    ...
                    res["vidpid"].as_array().emplace_back(
                        std::move(tmp_vidpid));
                } break;
    ...
    for (const UsbDevice &dev : active_devices_.value()) {
        ...
        try {
            ...
            Log::Error() << "Can't create a rule for device"
                         << "allow name "
                         << common_utils::QuoteIfNotQuoted(dev.name())
                         << " hash "
                         << common_utils::QuoteIfNotQuoted(dev.hash());
            Log::Error() << ex.what();
            throw;
        ...
        for (const auto &element : p_jobj_->at("deleted_rules").as_array()) {
            ...
            if (id_rule.has_value()) {
                ...
                throw std::runtime_error(
                    "Error parsing numeric rule number from json");
            }
        }
    }
    DeleteRules();
...
void JsonChanges::ProcessJsonAppended() {
    ...
    if (ptr_json_array_rules->empty()) return;
