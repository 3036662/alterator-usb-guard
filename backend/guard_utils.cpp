...
std::optional<std::vector<GuardRule>> UploadRulesCsv(
    ...
    try {
        ...
        for (size_t i = 0; i < n_rows; ++i) {
            try {
                std::string raw_str_tule =
                    utils::csv::CsvRule(doc, i).BuildString();
        ...
        if (!failed_rules.empty()) {
            Log::Warning() << "Parsing of " << failed_rules.size()
                           << "was failed";
            return std::nullopt;
        }
...
std::optional<std::string> BuildJsonArrayOfUpploaded(
    ...
    try {
        ...
        if (!vec_rules.empty()) common_target_policy = vec_rules[0].target();
        res_obj["policy"] = static_cast<uint>(common_target_policy);
        for (const auto &rule : vec_rules) {
            if (rule.target() != common_target_policy) found_conflicting = true;
            // Log::Debug() << rule.BuildJsonObject();
            switch (rule.level()) {
                case StrictnessLevel::hash:
                    res_obj.at("hash_rules")
                        .as_array()
                        .push_back(rule.BuildJsonObject());
                    break;
                case StrictnessLevel::interface:
                    res_obj.at("interf_rules")
                        .as_array()
                        .push_back(rule.BuildJsonObject());
                    break;
                case StrictnessLevel::vid_pid:
                    res_obj.at("vidpid_rules")
                        .as_array()
                        .push_back(rule.BuildJsonObject());
                    break;
                case StrictnessLevel::non_strict:
                    // must be empty for now - because loading of raw rules from
                    // csv is unsupported
...
bool VidPidValidator(const std::string &val) noexcept {
    ...
    for (const auto &element : spl) {
        ...
        try {
            size_t pos = 0;
            std::ignore = std::stoi(element, &pos, 16);
            if (pos != element.size()) throw std::logic_error("Not HEX number");
        } catch (const std::exception &ex) {
...
std::vector<std::string> SplitRawRule(std::string raw_str) noexcept {
    ...
    while (it_fast != raw_str.end()) {
        if (*it_fast == ' ' && !dont_split) {
            ...
            while (it_fast != raw_str.end() && *it_fast == ' ') ++it_fast;
            it_slow = it_fast;
        }
    ...
    if (it_fast == raw_str.end() && !dont_split && it_slow < it_fast)
        res.emplace_back(it_slow, it_fast);
    for (std::string &str : res) boost::trim(str);
    std::vector<std::string> tmp;
    for (auto &str : res) {
        if (!res.empty()) tmp.emplace_back(std::move(str));
    }
    std::swap(res, tmp);
    return res;
}
