...
ConfigStatus::ConfigStatus() noexcept
    : udev_warnings_{utils::InspectUdevRules()},
      udev_rules_OK_{udev_warnings_.empty()},
      guard_daemon_OK{false},
      guard_daemon_enabled_{false},
      guard_daemon_active_{false},
...
vecPairs ConfigStatus::SerializeForLisp() const {
    ...
    res.emplace_back("rules_file_exists",
                     rules_files_exists_ ? "TRUE" : "FALSE");
    ...
    res.emplace_back("config_files_permissions",
                     config_file_permissions_OK_ && rules_file_permissions_OK_
                         ? "OK"
                         : "BAD");
    res.emplace_back("audit_type", audit_backend_ == AuditType::kFileAudit
                                       ? "file"
                                       : "audit");
    return res;
}
...
std::optional<std::string> ConfigStatus::ExtractConfigFileName(
    const std::string &line) const noexcept {
    using common_utils::Log;
    std::string res;
    if (!boost::algorithm::contains(line, "ExecStart")) return std::nullopt;
    ...
    if (it_token != tokens.end()) ++it_token;
    ...
    if (boost::starts_with(line, "IPCAccessControlFiles=")) {
        ...
        if (pos < line.size()) {
            ...
            try {
                ...
                Log::Error()
                    << "Can't check users control folder" << path_to_folder;
                Log::Error() << ex.what();
            }
        }
        return true;
    ...
    if (boost::starts_with(line, "ImplicitPolicyTarget=")) {
        ...
        if (pos < line.size()) {
            ...
            if (!implicit_target.empty()) {
                ...
                Log::Info()
                    << "Implicit policy target = " << implicit_policy_target_;
            }
        }
        return true;
    }
...
void ConfigStatus::ParseDaemonConfig() noexcept {
    ...
    rules_files_exists_ = false;
    if (daemon_config_file_path_.empty()) return;
    // open config
    ...
    while (getline(file_dconfig, line)) {
        boost::trim(line);
        // rule file path
        if (ExtractRuleFilePath(line)) continue;
        // parse allowed user from conf file
        if (ExtractUsers(line)) continue;
        // parse allowed users folder and find all files in folder
        if (CheckUserFiles(line)) continue;
        // find all groups in config file
        if (ExtractGroups(line)) continue;
        // Audit type
        if (ExtractAuditBackend(line)) continue;
        if (ExtractAuditFilePath(line)) continue;
    ...
    try {
        if (const fs::path config_path(daemon_config_file_path_);
            ...
            if (const fs::perms conf_perm =
                ...
                config_file_permissions_OK_ = false;
                Log::Error()
                    << "USBGuard config file (" << daemon_config_file_path_
        ...
        if (fs::exists(daemon_rules_file_path)) {
            ...
            if (conf_perm == (fs::perms::owner_write | fs::perms::owner_read)) {
                ...
                rules_file_permissions_OK_ = false;
                Log::Error()
                    << "USBGuard rules file (" << daemon_rules_file_path
                    << ") must have 0600 permissions";
            }
        }
    } catch (const std::exception &ex) {
...
std::pair<std::vector<GuardRule>, uint> ConfigStatus::ParseGuardRulesFile()
    const noexcept {
...
bool ConfigStatus::OverwriteRulesFile(const std::string &new_content,
    ...
    if (rules_files_exists_) {
        ...
        if (parsed_new_file.first.size() != parsed_new_file.second ||
            !TryToRun(run_daemon)) {
            Log::Error()
                << "[OverwriteRulesFile] Error parsing new rules,old file "
                   "will be recovered";
            ...
            if (!TryToRun(run_daemon)) {
                Log::Error()
                    << "[OverwriteRulesFile] Can't start usbguard service. "
...
bool ConfigStatus::StartUsbguardDbus(
    ...
    if (start) {
        ...
        dbus_start_res = sysd.StartUnit(usb_guard_dbus_daemon_name);
        ...
        dbus_start_res = sysd.StopUnit(usb_guard_dbus_daemon_name);
...
bool ConfigStatus::EnableUsbguardDbus(
    ...
    if (enable) {
        ...
        dbus_enable_res = sysd.EnableUnit(usb_guard_dbus_daemon_name);
        ...
        dbus_enable_res = sysd.DisableUnit(usb_guard_dbus_daemon_name);
...
bool ConfigStatus::ChangeImplicitPolicy(bool block) noexcept {
    if (block && implicit_policy_target_ == "block") return true;
    if (!block && implicit_policy_target_ == "allow") return true;
    Log::Debug() << "Changing implicit policy to "
                 << (block ? "block" : "allow");
    try {
